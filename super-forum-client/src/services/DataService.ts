...
const user = new User("4", "test02@test.com", "tester02");
...
export async function getTopCategories(): Promise<Array<CategoryThread>> {
  const promise = new Promise<Array<CategoryThread>>((resolve, reject) => {
    setTimeout(() => {
      const topCategories = [];
...
/* export async function getThreadById(Id: string): Promise<Thread> {
  const promise = new Promise<Thread>((resolve, reject) => {
    setTimeout(() => {
      const thread = {
        id: "1",
        views: 22,
...
/* export async function getUserThreads(userId: string): Promise<Array<Thread>> {
  const promise = new Promise<Array<Thread>>((resolve, reject) => {
    setTimeout(() => {
      const threads: Array<Thread> = [];
      threads.push({
        id: "1",
